...
| 命令 | 说明 | 示例 |
|------|------|------|
| `/study-join` / `我想学习` / `加入学习` | 加入 MiniMind 学习计划（快捷指令） | `/study-join` |
| `/study minimind join` | 加入学习计划 | `/study minimind join` |
| `/study minimind status` | 查看学习进度 | `/study minimind status` |
| `/study minimind checkin [内容]` | 学习打卡 | `/study minimind checkin 完成了快速体验实验` |
| `/study minimind reminder [on/off]` | 开关学习提醒 | `/study minimind reminder off` |
| `/study minimind leave` | 退出学习计划 | `/study minimind leave` |

### 学习流程

1. **加入学习计划**：发送 `/study-join` 或 `/study minimind join`
2. **Bot 返回**：第一个学习任务（模块、步骤、预计耗时）
3. **完成学习**：按照学习路径完成内容
4. **学习打卡**：发送 `/study minimind checkin [学习心得]`
5. **Bot 记录**：更新进度并解锁下一个任务
6. **定时督促**：每天 19:00 自动发送学习提醒（如已开启提醒）
...
- ✅ 原理优先，而非命令复制
- ✅ 对照实验验证每个设计选择
- ✅ 可在普通笔记本运行（每个实验 < 10 分钟）
- ✅ 搭配 Claude Code 半自动化学习流

**督促特点：**
- ⏰ 每天 19:00 自动提醒（与通用打卡督促并行）
- 👨‍🏫 Teacher 风格的循循善诱
- 📊 模块级别的进度追踪
- 💡 个性化的学习建议
