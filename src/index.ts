...
interface YearlyReportData {
  ...
  longestSingleDay: number;
  longestSingleDayDate: string;
  categoryStats: Array<{ category: string; minutes: number; percentage: number }>;
  allCheckins: Array<{
    date: string;
    content: string;
    duration: number;
...
// 查询用户年度打卡数据
async function getYearlyReportData(userId: number, year: number): Promise<YearlyReportData | null> {
  ...
  if (yearCheckins.length === 0) return null;

  // 过滤正常打卡
  const normalCheckins = yearCheckins.filter(c => !c.isLoan);

  if (normalCheckins.length === 0) return null;

  // 计算基础统计
  const totalMinutes = normalCheckins.reduce((sum, c) => sum + c.duration, 0);
  const totalCheckins = normalCheckins.length;

  // 计算打卡天数（去重）
  const uniqueDays = new Set(
    normalCheckins.map(c => c.createdAt.toISOString().split('T')[0])
  ).size;

  // 计算单日最长打卡
  const dailyStats = new Map<string, number>();
  normalCheckins.forEach(c => {
    const date = c.createdAt.toISOString().split('T')[0];
    dailyStats.set(date, (dailyStats.get(date) || 0) + c.duration);
  });
  const [longestDate, longestSingleDay] = [...dailyStats.entries()]
    .sort((a, b) => b[1] - a[1])[0];
  ...
  normalCheckins.forEach(c => {
    ...
    categoryMinutes[cat] = (categoryMinutes[cat] || 0) + c.duration;
  });

  const categoryStats = Object.entries(categoryMinutes)
  ...
  // 整理所有打卡记录（用于 AI 分析）
  const allCheckins = normalCheckins.map(c => ({
    date: c.createdAt.toLocaleDateString('zh-CN'),
    content: c.content,
    duration: c.duration,
    category: c.category
  ...
  // 筛选年度成就
  const yearAchievements = user.achievements.filter(a =>
    a.unlockedAt >= yearStart && a.unlockedAt < yearEnd
  );
  ...
  return {
    ...
    longestSingleDay,
    longestSingleDayDate: longestDate,
    categoryStats,
    allCheckins,
    achievements: yearAchievements,
    avgDailyMinutes: Math.round(totalMinutes / uniqueDays)
  };
}

// 生成年度报告 AI 深度分析
async function generateYearlyAISummary(
  nickname: string,
  data: YearlyReportData,
  aiStyle: string
): Promise<string | null> {
  if (!AI_API_KEY || data.totalCheckins < 3) {
    return null;
  }

  const style = getAIStyle(aiStyle);

  // 构建打卡记录摘要（最多 20 条，避免 token 过长）
  const checkinsSummary = data.allCheckins
    .slice(-20)
    .map(c => `- ${c.date}: ${c.content} ${formatDuration(c.duration)}`)
    .join('\n');

  // 分类统计字符串
  const categoryStr = data.categoryStats
...
你正在生成用户的年度打卡总结，这是一份年终回顾。

要求：
- 基于你的风格给予回应
- 500 字左右，分成 3-4 段
- 第一段：总结用户今年的整体表现，肯定用户的努力和坚持
- 第二段：分析用户的兴趣方向和习惯（从打卡内容和分类）
- 第三段：给出明年的建议和展望，要有鼓励性
- 要有仪式感，这是年度报告！
- 用"你"而不是"您"，语气要真诚`;

  const userPrompt = `用户「${nickname}」的 ${data.year} 年度打卡数据：

【基础数据】
- 累计打卡: ${formatDuration(data.totalMinutes)}
- 打卡次数: ${data.totalCheckins} 次
- 打卡天数: ${data.uniqueDays} 天
- 日均时长: ${formatDuration(data.avgDailyMinutes)}

【连续打卡】
- 当前连续: ${data.currentStreak} 天
- 历史最长: ${data.maxStreak} 天

【高光时刻】
- 单日最长: ${formatDuration(data.longestSingleDay)} (${data.longestSingleDayDate})
...
请用你的风格生成一份有仪式感的年度总结和新年寄语。`;
  ...
  try {
    ...
    if (!response.ok) {
      console.error('年度报告 AI API 错误:', response.status);
      return null;
    }
    ...
    console.error('年度报告 AI 调用失败:', error);
    return null;
  }
}
...
async function generateYearlyReport(
  ...
  if (!user) {
    sendReply(ws, event, '你还没有打卡记录哦～');
    return;
  }

  // 获取年度数据
  const data = await getYearlyReportData(user.id, reportYear);

  if (!data) {
    sendReply(ws, event, `${reportYear} 年还没有打卡记录哦～`);
    return;
  }

  // 构建报告消息
  let message = `📊 ${user.nickname} 的 ${reportYear} 年度报告\n\n`;

  // 基础统计
  message += `📈 基础统计\n`;
  message += `├ 累计打卡: ${formatDuration(data.totalMinutes)}\n`;
  message += `├ 打卡次数: ${data.totalCheckins} 次\n`;
  message += `├ 打卡天数: ${data.uniqueDays} 天\n`;
  message += `└ 日均时长: ${formatDuration(data.avgDailyMinutes)}\n\n`;

  // 连续打卡
  message += `🔥 连续打卡\n`;
  message += `├ 当前连续: ${data.currentStreak} 天\n`;
  message += `└ 历史最长: ${data.maxStreak} 天\n\n`;

  // 高光时刻
  message += `✨ 高光时刻\n`;
  message += `├ 单日最长: ${formatDuration(data.longestSingleDay)}\n`;
  message += `└ 日期: ${data.longestSingleDayDate}\n\n`;

  // 分类分布
  if (data.categoryStats.length > 0) {
    message += `📚 分类分布\n`;
    data.categoryStats.slice(0, 5).forEach((cat, i) => {
      const bar = '█'.repeat(Math.floor(cat.percentage / 10)) + '░'.repeat(10 - Math.floor(cat.percentage / 10));
      message += `${i + 1}. ${cat.category} ${bar} ${cat.percentage}%\n`;
    });
    message += `\n`;
  }
  ...
  // AI 深度分析
  const aiSummary = await generateYearlyAISummary(user.nickname, data, user.aiStyle);
  if (aiSummary) {
    message += `🤖 AI 深度分析:\n${aiSummary}`;
  } else {
    message += `🤖 AI 分析暂时不可用，数据不足请稍后再试～`;
  }
...
// 年度报告配置
const YEARLY_REPORT_MONTH = parseInt(process.env.YEARLY_REPORT_MONTH || '12');
const YEARLY_REPORT_DAY = parseInt(process.env.YEARLY_REPORT_DAY || '31');
const YEARLY_REPORT_HOUR = parseInt(process.env.YEARLY_REPORT_HOUR || '20');
const YEARLY_REPORT_MINUTE = parseInt(process.env.YEARLY_REPORT_MINUTE || '0');

// 获取下次年度报告推送时间（毫秒）
function getNextYearlyReportTime(): number {
  const now = new Date();
  const targetTime = new Date(now.toLocaleString('en-US', { timeZone: REMINDER_TIMEZONE }));

  const reportTime = new Date(targetTime);
  reportTime.setMonth(YEARLY_REPORT_MONTH - 1, YEARLY_REPORT_DAY);
  reportTime.setHours(YEARLY_REPORT_HOUR, YEARLY_REPORT_MINUTE, 0, 0);

  // 如果今年的时间已过，设置为明年
  if (reportTime <= targetTime) {
    reportTime.setFullYear(reportTime.getFullYear() + 1);
  }

  const nowInTimezone = new Date(now.toLocaleString('en-US', { timeZone: REMINDER_TIMEZONE }));
  return reportTime.getTime() - nowInTimezone.getTime();
}

// 发送私聊消息
function sendPrivateMessage(ws: WebSocket, userId: string, message: string): void {
  const msg = {
...
// 启动年度报告自动推送定时器
function startYearlyReportScheduler(ws: WebSocket): void {
  if (!REMINDER_GROUP_ID) {
    console.log('年度报告功能未配置（需要 REMINDER_GROUP_ID）');
    return;
  }

  const scheduleNextReport = () => {
    const delay = getNextYearlyReportTime();
    const nextTime = new Date(Date.now() + delay);

    console.log(`下次年度报告推送时间: ${nextTime.toLocaleString('zh-CN', { timeZone: REMINDER_TIMEZONE })} (${REMINDER_TIMEZONE})`);
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(`准备为 ${activeUsers.length} 位用户发送 ${year} 年度报告`);

        for (const user of activeUsers) {
          try {
            const data = await getYearlyReportData(user.id, year);

            if (!data) continue;

            let message = `📊 ${user.nickname} 的 ${year} 年度报告\n\n`;
            message += `📈 基础统计\n`;
            message += `├ 累计打卡: ${formatDuration(data.totalMinutes)}\n`;
            message += `├ 打卡次数: ${data.totalCheckins} 次\n`;
            message += `├ 打卡天数: ${data.uniqueDays} 天\n`;
            ...
            sendPrivateMessage(ws, user.qqNumber, message);
            console.log(`已发送年度报告给 ${user.nickname} (${user.qqNumber})`);

            // 间隔 2 秒避免 API 限流
            await new Promise(resolve => setTimeout(resolve, 2000));
          } catch (error) {
            console.error(`为用户 ${user.qqNumber} 生成年度报告失败:`, error);
          }
        }

        console.log('年度报告推送完成');
      } catch (error) {
        console.error('年度报告推送失败:', error);
      }
  ...
  ws.on('message', async (data) => {
    try {
      ...
      switch (command) {
        ...
        case '年报':
        case '我的年报':
          await generateYearlyReport(ws, event, event.user_id!);
          break;
