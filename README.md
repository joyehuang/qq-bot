...
2. **断签结算**（已经断了）
   - 每天 **北京时间 00:01** 跑一次：扫所有 `streakDays >= 1` 的用户，昨天没打卡的全部清零
   - 其中原 streak ≥5 的用户会收到 @ 调侃通知
   - 这一步是「主动结算」，确保 `streakDays` 字段在第二天起就是准确的（修复了过去依赖 filter 导致的"已断却仍提醒要断"bug）

### 调度器与时区

- 所有定时任务统一由 [`node-cron`](https://www.npmjs.com/package/node-cron) 注册，cron 表达式按 **北京时间（Asia/Shanghai）** 解析，与服务器物理时区无关
- 重连时会自动停掉旧 cron 并重新注册，避免重复触发
- 进程重启后 cron 会按下次到期时间重新调度（错过当前窗口的任务不会回放）

### 定时消息的 AI 化

打卡督促 / 断签警告 / 断签调侃这三类定时消息默认走 **AI 生成**（Hermes Agent + 用户最近 7 天打卡历史 + 用户的 `aiStyle` 偏好），AI 调用失败时自动回退到内置模板池。

- **个性化**：每条消息都基于该用户自己的打卡历史和风格偏好生成
- **健壮**：Hermes 不可用 / 超时 / 输出过短 时自动 fallback 到模板，不会卡死定时任务
- **不发送内部 prompt**：发到群里的只有 AI 输出本身，且系统会自动确保消息开头有 `[CQ:at,qq=...]` 标签

### 群周报

每周一 09:00（北京时间）自动汇总「上周一 0:00 至本周一 0:00」的群整体打卡数据，并由 AI 生成总结发送到群里。包含：

- 整体表现：活跃用户数、总打卡次数、总时长
- TOP 5 用户排行
- 分类分布（按时长占比）
- AI 观察：80–150 字的整体趋势总结 + 下周鼓励
